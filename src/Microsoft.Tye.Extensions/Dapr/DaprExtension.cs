            ...
            if (context.Operation == ExtensionContext.OperationKind.LocalRun)
            {
                ...
                foreach (var project in projects)
                {
                    ...
                    var proxy = new ExecutableServiceBuilder($"{project.Name}-dapr", "daprd")
                    {
                        ...
                        Args = $"-app-id {project.Name} -app-port %APP_PORT% -dapr-grpc-port %DAPR_GRPC_PORT% --dapr-http-port %DAPR_HTTP_PORT% --metrics-port %METRICS_PORT% --placement-address localhost:6050",
                    };
